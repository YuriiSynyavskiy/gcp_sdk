...
from dist.sql_queries import sql_landing_to_staging_person, sql_staging_to_target_person, sql_temporary_to_landing_person
from dist.utils import define_file, check_file_existing, check_more_files, message_logging

LOG_NAME = 'person-scd-dag'
...
with airflow.DAG(
    ...
    start_of_job = PythonOperator(
        ...
        op_args=[logger, 'INFO', f'{datetime.now(tz=None)} Starting of job with id {{{{run_id}}}} for processing persons data'],
        dag=dag
    )
    ...
    check_file = BranchPythonOperator(
        ...
        op_kwargs={'namespace': 'persons', 'LOG_NAME': LOG_NAME},
        dag=dag,
    )
    ...
    load_csv = GCSToBigQueryOperator(
        ...
        destination_project_dataset_table=f"{Variable.get('LANDING_DATASET_ID')}.{tables.tmp_person_table_name}",
        write_disposition='WRITE_TRUNCATE',
        skip_leading_rows=1,
        schema_fields=[
            {'name': 'person_key', 'type': 'STRING', 'mode': 'REQUIRED'},
    ...
    log_on_failure_file_to_tmp = PythonOperator(
        ...
        op_args=[logger, 'ERROR', 
            f"{datetime.now(tz=None)} Error while loading file {{{{ ti.xcom_pull(task_ids='define_file_for_uploading')}}}} to {Variable.get('LANDING_DATASET_ID')}.{tables.tmp_person_table_name}. Job id - {{{{run_id}}}}"],
    ...
    log_on_failure_tmp_to_lnd = PythonOperator(
        ...
        python_callable=message_logging,
        op_args=[logger, 'ERROR', 
            f"{datetime.now(tz=None)} Error while moving department data from {Variable.get('LANDING_DATASET_ID')}.{tables.tmp_person_table_name} to {Variable.get('LANDING_DATASET_ID')}.{tables.person_table_name}. Job id - {{{{run_id}}}}"],
        trigger_rule='one_failed',
        dag=dag
    )
    ...
    log_on_failure_lnd_to_stg = PythonOperator(
        ...
        python_callable=message_logging,
        op_args=[logger, 'ERROR', 
            f"{datetime.now(tz=None)} Error while moving department data from {Variable.get('LANDING_DATASET_ID')}.{tables.person_table_name} to {Variable.get('STAGING_DATASET_ID')}.{tables.person_table_name}. Job id - {{{{run_id}}}}"],
        trigger_rule='one_failed',
        dag=dag
    )
    ...
    log_on_failure_stg_to_trg = PythonOperator(
        ...
        python_callable=message_logging,
        op_args=[logger, 'ERROR', 
            f"{datetime.now(tz=None)} Error while moving department data from {Variable.get('STAGING_DATASET_ID')}.{tables.person_table_name} to {Variable.get('DATASET_ID')}.{tables.person_table_name}. Job id - {{{{run_id}}}}"],
        trigger_rule='one_failed',
        dag=dag
    )
    ...
    check_for_another_file = BranchPythonOperator(
        ...
        op_kwargs={'destination_object': "processed_{{ti.xcom_pull(task_ids='define_file_for_uploading')}}",
                   ...
                   'namespace': 'persons', 'LOG_NAME': LOG_NAME},
        dag=dag,
    )
